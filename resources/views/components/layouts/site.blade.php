        ...
        <ul role="list" class="flex flex-1 flex-col gap-y-4">
            <li>
                <ul role="list" class="-mx-2 space-y-1">
                    <li>
                        <x-nav-link href="{{ route('sites.show', ['site' => $site->id]) }}" class="flex items-center gap-x-2.5" :active="request()->routeIs('sites.show')">
            ...
            <li>
                <div class="text-gray-700 flex items-center justify-between">
                    ...
                    <x-link to="/sites/{{ $site->id }}/create-endpoint" class="flex items-center hover:text-indigo-500">
                ...
                <ul role="list" class="-mx-2 space-y-1 mt-2">
                    @foreach($site->endpoints as $endpoint)
                        <li wire:key="{{ $endpoint->id }}">
                            <x-nav-link href="{{ route('endpoints.show', ['endpoint' => $endpoint]) }}" class="flex items-center gap-x-2.5" :active="request()->is('endpoints/' . $endpoint->id)">
